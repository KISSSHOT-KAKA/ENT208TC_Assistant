...
ENT208TC Assistant 是一个面向有桌面场景长期需求的智能语音伙伴系统。它将 **M5Stack AtomS3R 智能硬件终端**、**Ubuntu 串口事件桥接服务**、**Windows Electron 桌面应用** 和 **Live2D Cubism 虚拟角色界面**组合在一起，让语音交互不只停留在设备端（AtomS3 R），而是可以实时驱动一个具有状态、表情、动作和对话反馈的桌面虚拟伙伴。
